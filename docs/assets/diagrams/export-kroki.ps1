...
function Export-Kroki($name, $content) {
    $out = Join-Path $dir "$name.png"
    $body = $content.Trim()
    try {
        Invoke-WebRequest -Uri "https://kroki.io/mermaid/png" -Method POST -Body $body -ContentType "text/plain" -OutFile $out -UseBasicParsing
        $size = (Get-Item $out).Length
        if ($size -lt 1000) { throw "File too small ($size bytes)" }
...
Export-Kroki "01-reporter-journey" @'
flowchart LR
    subgraph S1[会前准备]
...
Export-Kroki "02-manager-journey" @'
flowchart LR
    subgraph M1[订阅关注]
...
Export-Kroki "03-architecture-layers" @'
flowchart TB
    subgraph APP[应用层 Application Layer]
        direction LR
        A1[会议导入] --- A2[智能纪要] --- A3[任务优先级] --- A4[汇报优化] --- A5[任务中心]
    end
    subgraph RULE[规则引擎层 Rule Engine Layer]
        direction LR
...
Export-Kroki "05-mechanism-structural" @'
flowchart TD
...
Export-Kroki "06-mechanism-four-elements" @'
flowchart TD
...
Export-Kroki "07-mechanism-confidence" @'
flowchart TD
...
Export-Kroki "08-mechanism-sync" @'
flowchart TD
...
Export-Kroki "09-core-interaction-flow" @'
flowchart TD
    A[会议录音] --> B[语音转写层] --> F[实时转写文本]
    F --> G[大模型语义理解] --> H[结构化抽取] --> I[规则引擎校验]
    I -->|通过| K[自动标记已确认]
    I -->|不通过| L[标记待确认]
    K --> M[智能纪要展示]
    L --> M
    M --> P[人工确认] --> S[任务自动拆解]
    S --> U[四象限优先级初判] --> V[AI任务看板]
    V --> Y[同步前置校验]
    Y -->|通过| AC[飞书钉钉同步]
    Y -->|不通过| Z[提示补全字段]
    AC --> AG[任务中心追踪]
