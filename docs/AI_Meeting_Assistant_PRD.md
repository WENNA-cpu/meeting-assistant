...
| v2.1 | 2026-06-22 | 杨文娜 | 补充技术实现、已实现功能清单、v1.1 待完善功能 |
| v2.2 | 2026-06-22 | 杨文娜 | 第 11、12 章改为流程图展示 |
| v2.2.1 | 2026-06-22 | 杨文娜 | 修复第 11、12 章流程图中文乱码（???） |
| v3.0 | 2026-06-22 | 杨文娜 | 企业级落地补充：商业化、竞品、技术依赖、权限模型、兜底策略 |
...
- 单场会议纪要整理时间从平均 90 分钟缩短至 15 分钟以内（可追踪）
- 因会议待办不清晰导致的重复沟通/跟进度数下降 ≥ 30%（定性追踪）
- 汇报材料准备时间减少 ≥ 50%（用户自评）
- 任务逾期率下降 ≥ 20%（对比使用前后）
...
| 竞品 | 核心能力 | 主要不足 |
|------|----------|----------|
| 通义听悟 | 实时转写、摘要提取 | 缺少任务分级与协作同步，不做汇报优化 |
| 讯飞听见 | 高精度转写、多语言 | 偏转写工具，结构化能力弱，无任务闭环 |
| Otter.ai | 实时转写、团队协作 | 对中文支持一般，缺少企业级权限与私有化 |
| 飞书妙记 | 与飞书深度集成 | 生态绑定强，跨平台能力弱，任务管理浅 |

### 4.2 核心差异化优势

- **全链路闭环**：从录音到任务同步，一站完成，而非单点转写工具。
- **人机协同设计**：每个关键环节（纪要确认、任务校验）都保留人工裁量权，适合企业严肃场景。
- **汇报优化专属能力**：面向向上汇报场景的话术润色与结构调整，是现有竞品普遍缺失的。
- **中立跨平台**：同时支持飞书和钉钉，不绑定单一协作生态。
- **四象限任务管理**：将会议待办按紧急/重要矩阵可视化，直接对齐项目管理最佳实践。
...
| **IT/合规** | 企业 IT 管理员、安全合规负责人 | 数据安全可控，满足内部合规与审计要求，便于采购决策 |
...
#### 场景 F：合规与采购评估（IT/合规负责人）

IT 管理员评估产品是否符合企业数据驻留、认证集成、审计日志等要求，顺利完成采购审批。
...
**IT 管理员主路径**：评估安全合规 → 配置 SSO/权限 → 监控使用与数据留存
...
| 部署方式 | 说明 | 适用客户 |
|----------|------|----------|
| SaaS 公有云（默认） | 数据存储于云服务商，多租户隔离 | 一般企业、快速上线需求 |
| 混合部署（可选） | 转写与 LLM 能力可配置为本地或专有云，纪要数据落客户侧 | 金融、政务、大型跨国企业 |

V1.0 优先支持 SaaS 模式，V1.1 提供私有化部署方案评估与报价。

### 7.2 定价模型建议（供商业评审参考）

| 定价维度 | 建议 | 说明 |
|----------|------|------|
| 按用户席位 | 按汇报者（编辑者）席位包月/包年，管理者只读账号免费或优惠 | 主收入来源 |
| 按会议时长 | 超出套餐音频分钟数后按量计费 | 弹性收入 |
| 按企业层级 | 团队版、部门版、企业版（含私有化）分级定价 | 差异化 |

具体定价需与商业化团队联合确定，PRD 阶段仅做框架性约束：必须支持按席位的计费模型，且能够统计每个企业的会议音频处理量。

### 7.3 采购与合规关键约束

| 维度 | 要求 |
|------|------|
| 认证集成 | 支持 SAML 2.0 / OIDC 对接企业 AD / LDAP / 钉钉 / 飞书 SSO |
| 数据驻留 | 支持按企业指定数据存储地域（如中国境内/境外） |
| 审计日志 | 关键操作（确认纪要、同步任务、导出汇报）须有操作日志，保留至少 180 天 |
| 数据删除 | 支持企业按策略配置数据自动清理（如 90 天/180 天/永久） |
| 合规 | 满足《个人信息保护法》及企业内部数据出境管理规范 |
...
| 上传中断或网络失败 | 自动断点续传，或提示「网络异常，请稍后重试」 |
...
| 维度 | 要求 |
|------|------|
| 数据传输 | 会议音频及纪要数据全程加密传输（TLS 1.2+） |
| 数据存储 | 企业数据隔离存储，支持按企业策略配置保留周期 |
| 权限控制 | 会议与任务按企业组织架构与角色授权访问（详见 11.5） |
| 审计留痕 | 关键操作（确认纪要、同步任务、导出汇报）留痕可追溯，保留 ≥ 180 天 |
...
基于企业组织架构的 RBAC（基于角色的访问控制）模型：

| 角色 | 可见范围 | 可编辑范围 | 可操作范围 |
|------|----------|------------|------------|
| 汇报者（编辑者） | 自己发起的会议及任务 | 自己发起的会议纪要、任务 | 确认纪要、分配任务、同步协作 |
| 管理者（只读+审阅） | 所辖团队所有会议纪要及任务 | 不可编辑原始纪要，可添加批注/审阅意见 | 查看所有、审阅汇报稿、标记决策确认 |
| IT 管理员 | 全局配置及审计日志 | 系统配置、权限分配、数据留存策略 | 不涉及业务数据编辑 |

权限颗粒度细化到「会议级别」和「任务级别」，支持后续按部门、项目组扩展。
...
| 失效场景 | 兜底操作 |
|----------|----------|
| ASR 转写完全失败 | 允许用户上传本地已有转写文稿（txt/srt），跳过转写直接进入结构化 |
| LLM 结构化完全失败 | 展示原始转写全文，用户可手动创建结构化条目（决策/分工/待办/问题） |
| 汇报优化生成失败 | 保留原始会议原文，支持用户手动编辑后导出 |

### 12.2 协作同步失败的兜底

| 场景 | 兜底方案 |
|------|----------|
| 飞书/钉钉 API 不可用 | 本地任务状态保留，定时重试（最多 24 小时），超时后标记为「同步异常，请手动处理」 |
| 用户授权过期 | 在任务中心顶部常驻提示「协作授权已过期」，点击后引导重新授权 |
| 部分任务同步成功、部分失败 | 展示详细的成功/失败清单，支持用户勾选失败项单独重试，或导出 CSV 手动导入 |

### 12.3 数据恢复与回滚

- 每次用户「确认」操作前，系统自动保存当前版本的纪要/任务快照。
- 支持用户在 30 天内回滚到任意历史快照版本（V1.0 支持最近 3 个版本）。
- 删除的会议和任务进入「回收站」，保留 30 天后彻底清除（企业可配置）。

---

## 13. 版本规划

### 13.1 V1.0（MVP）— 当前目标版本

**目标**：打通「会议导入 → 智能纪要 → 任务优先级 → 汇报优化 → 任务中心」主链路，并完成飞书/钉钉基础同步。
...
![会议助手技术实现图](./images/会议助手_技术实现.png)

> 图：语音转写（faster-whisper）→ 结构化纪要（DeepSeek）→ 任务拆解（四象限）三层技术实现。
...
| 项目 | 说明 |
|------|------|
| 技术方案 | 本地 faster-whisper 模型（离线部署），不依赖外部 ASR API |
| 支持语言 | 中文普通话为主，英文为辅（V1.0 侧重中文） |
| 已知局限 | 专业术语（金融、医疗、IT 黑话）识别准确率可能下降；多人同时发言时分离效果受限 |
| 兜底方案 | 若转写置信度低于阈值，标记「转写质量较低，建议人工核对」；后续可切换云端 ASR（如阿里/讯飞）作为备选 |
| 关键依赖 | 需要 GPU 算力资源（或 CPU 优化版本），部署前需评估服务器配置 |

### 15.2 结构化纪要（LLM）

| 项目 | 说明 |
|------|------|
| 技术方案 | DeepSeek API（云端调用） |
| 数据安全 | 若企业要求数据不出域，需支持 DeepSeek 私有化部署或切换为同级别国产大模型（如通义千问、智谱 GLM） |
| 已知局限 | 对于超长会议（>2 小时），结构化抽取可能遗漏尾部内容；需做文本截断或分段处理 |
| 降级方案 | 若 LLM API 不可用，降级为基于规则的关键词匹配抽取（仅做兜底，质量较低） |
| 关键依赖 | 依赖 DeepSeek API 的可用性与响应延迟，需与供应商签订 SLA |

### 15.3 任务拆解与优先级计算

| 项目 | 说明 |
|------|------|
| 技术方案 | 规则引擎（基于关键词 + 正则）+ LLM 辅助分类 |
| 优先级初判 | 基于规则计算（紧急/重要标记词），低置信度时标记「AI 建议，请确认」 |
| 关键依赖 | 无外部依赖，纯规则引擎可离线运行 |

### 15.4 协作平台同步

| 项目 | 说明 |
|------|------|
| 技术方案 | 飞书开放 API / 钉钉开放 API |
| 支持操作 | 创建任务、更新状态、添加评论（以平台开放能力为准） |
| 已知限制 | 飞书/钉钉有 API 调用频次限制（QPS），大批量同步需做排队与限流 |
| 降级方案 | 同步失败时本地保留，展示「待同步」状态，支持用户手动重试或导出 |
| 关键依赖 | 需提前申请飞书/钉钉企业应用权限与 API 配额 |

---

## 16. 已实现功能清单

![会议助手已实现功能图](./images/会议助手_已实现功能.png)

> 图：主链路全流程已实现，配套多会议管理与删除会议能力。（具体以开发团队实际交付为准）

---

## 17. 待完善功能（v1.1 规划）

- 真实音频转写接入（Whisper 服务优化）
- 飞书/钉钉任务同步（API 对接）
- 实时录音支持
- 批量任务操作
